...
CONFIGURE_FLAGS="--with-mpi --with-shared-libraries=0 --download-fblaslapack=yes"
...
build_petsc()
{
	...
	if $ARCH || $MANJARO
	then
		run_command python2 ./configure CC=$CC CXX=$CXX FC=$FC F77=$F77 F90=$F90 --prefix=$LIBRARY_BUILD_TYPE_INSTALL_DIR $CONFIGURE_FLAGS
	else
		run_command ./configure CC=$CC CXX=$CXX FC=$FC F77=$F77 F90=$F90 --prefix=$LIBRARY_BUILD_TYPE_INSTALL_DIR $CONFIGURE_FLAGS
	fi
